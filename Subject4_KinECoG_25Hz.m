...
% subject 4: load for ECoG 25Hz
load('/media/reza/WindowsDrive/ECoGLeapMotion/DataPatientFour/ECoGData/ecog_kinematics_25Hz.mat')
clear fingers hand

% subject 4: load for kinematics 25Hz
load('/media/reza/WindowsDrive/ECoGLeapMotion/DataPatientFour/ECoGData/kinData_EC189_25Hz.mat')
...
for Movement = 1:8 
    ...
    Kin_Data_Filtered = filtfilt(b,a,Kin_Data);
    Kin_Data_Smoothed = movmean(Kin_Data_Filtered,10);
    %figure;
    %plot(Kin_Data_Smoothed)
    
    if Movement == 1
        Kin_Data_Modified = Kin_Data_Smoothed(241:2131,:);
        
    elseif Movement == 2
        Kin_Data_Modified = Kin_Data_Smoothed(113:1100,:);
        
    elseif Movement == 3
        Kin_Data_Modified = Kin_Data_Smoothed(109:757,:);
        
    elseif Movement == 4
        Kin_Data_Modified = Kin_Data_Smoothed(189:1000,:);
    
    elseif Movement == 5
        Kin_Data_Modified = Kin_Data_Smoothed(109:1223,:);
    
    elseif Movement == 6
        Kin_Data_Modified = Kin_Data_Smoothed(161:1291,:);
    
    elseif Movement == 7
        Kin_Data_Modified = Kin_Data_Smoothed(255:1400,:);        
    
    elseif Movement == 8
        Kin_Data_Modified = Kin_Data_Smoothed(131:1785,:);
        
    end
    
    % PCA per movement
    [coeff,score,latent,tsquared,explained] = pca(Kin_Data_Modified);
    %figure;
    %plot(score(:,1),'b')
    %hold on
    %plot(diff(score(:,1)),'r')
...
%% Finger 1 position
half_window=30; %points; about 1 sec: for finding local max and local min in position data
...
j=1;
jj=1;
for i=3*half_window:F1_n-(3*half_window+1)
...
%% Finger 2 position
half_window=25; %points; about 1 sec: for finding local max and local min in position data
...
j=1;
jj=1;
for i=2*half_window:F2_n-(half_window+1)
...
Index_F2=sort([34; FingersKinInfo.Finger(2).Fs_MaxPos;...
    FingersKinInfo.Finger(2).Es_MaxPos; F2_n]);

AbsVel_F2 = (PC_Vel(2).Movement); 
...
%% Finger 3 position
half_window=20; %points; about 1 sec: for finding local max and local min in position data
...
j=1;
jj=1;
for i=2*half_window:F3_n-(half_window+1)
...
j=1;
jj=1;
for i=2*half_window:F4_n-(half_window+1)
...
j=1;
jj=1;
for i=2*half_window:F5_n-(half_window+1)
...
%% Grasp 1 position
half_window=18; %points; about 1 sec: for finding local max and local min in position data
...
j=1;
jj=1;
for i=2*half_window:F6_n-(half_window+1)
...
j=1;
jj=1;
for i=2*half_window:F7_n-(half_window+1)
...
for Fi=1:8
    ...
    if Fi == 1
        Signal_Modified = Signal(241:2131,:);
        Finger_Sizes(2) = length(Signal_Modified);
        
    elseif Fi == 2
       Signal_Modifiedd = Signal(113:1100,:);
       Finger_Sizes(3) = length(Signal_Modified);
        
    elseif Fi == 3
        Signal_Modified = Signal(109:757,:);
        Finger_Sizes(4) = length(Signal_Modified);
        
    elseif Fi == 4
        Signal_Modified = Signal(189:1000,:);
        Finger_Sizes(5) = length(Signal_Modified);
        
    elseif Fi == 5
        Signal_Modified = Signal(109:1223,:);
        Finger_Sizes(6) = length(Signal_Modified);
        
    elseif Fi == 6
        Signal_Modified = Signal(161:1291,:);
        Finger_Sizes(7) = length(Signal_Modified);
        
    elseif Fi == 7
        Signal_Modified = Signal(255:1400,:);
        Finger_Sizes(8) = length(Signal_Modified);
        
    elseif Fi == 8
        Signal_Modified = Signal(131:1785,:);
        Finger_Sizes(9) = length(Signal_Modified);
        
    end
    
    % for hg-lfo
    F_Envelope = Signal_Modified(:,1:256); 
...
for Fi=1:8
    ...
    lfo_hilbert{1,Fi} = Signal_Modified(:,1:256);
...
% return data to the same structures using Finger_Sizes
...
save('/media/reza/WindowsDrive/ECoGLeapMotion/ResultsGroupAnalysis/github_Branch_V3/Subject4_NikMethod_25Hz.mat',...
    'ECoG_lfo','ECoG_hglfo');
