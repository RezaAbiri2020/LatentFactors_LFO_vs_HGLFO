...
function Rsq_Pre_output=MultipleRegFuncLag(Finger,Input_dynamics,Output_Dynamics,LagN,Figures)
...
if Lag_Number == 0
    ...
    Lagged_Projs=Input_dynamics;
    for i=1:Lag_Number
        Input1 = circshift(Input_dynamics,i);
        %Input2 = [zeros(i,size(Input_dynamics,2));Input1(i+1:end,:)];
        %Lagged_Projs = [Lagged_Projs, Input2];
        Lagged_Projs = [Lagged_Projs, Input1];
    end
...
R_Pre_final=corr(Lagged_Kins_S(200:end),velPredict_cv(200:end));
Rsq_Pre_output=R_Pre_final^2;
